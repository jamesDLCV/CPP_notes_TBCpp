...
int main()
{
    ...
    std::vector<int> my_arr4{ 1, 2, 3 };        // uniform initialization도 된다. 

    cout << my_arr4.size() << endl;             // 3
    cout << sizeof(my_arr4) << endl;            // 16(x86), 32(x64) <- sizeof()
    cout << endl;

    // 위와 같이 사이즈가 다양한 것들을 쉽게 할 수 있다. 
    // 더 좋은 것은 사이즈를 그때 그때 바꿀 수가 있다는 것이다. 저 밑에 예제 제시함. 
    ...
    // 이 vector의 장점은 메모리를 알아서 지워준다는 것이다. 

    int* my_arr6 = new int[5];  // 이렇게 했으면

    delete[] my_arr6;           // 이렇게 꼭 지워라. 
                                // 이렇게 꼭 지워야 한다는 게 코딩할 때는 부담이다. 
    ...
    /*
    int *my_arr = new int[5]; 이와 같은 동적 할당을 직접 코딩해서 만들려면,     
    원래 있던 메모리 주소, 사용하는 메모리의 길이를 OS에게 물어 봐서,
